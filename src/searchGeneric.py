...
class Searcher(Displayable):
    """returns a searcher for a ploblem.
    Paths can be found by repeatedly calling search().
    This does depth-first search unless overriden
    """
    ...
    def search(self):
        ...
        to a goal node. Returns None if no more paths"""
        while not self.empty_frontier():
            ...
            if self.problem.is_goal(self.path.end()): # solution found
                ...
                self.display(1, f"Solution: {self.path} (cost: {self.path.cost})\n",
                             self.num_expanded, "paths have been expanded and", len(self.frontier), "paths remain in the frontier")
                return self.path
            else:
...
class FrontierPQ(object):
    """A frontier consists of a priority queue (heap), frontierpq, of (value, index, path) triples, where
    ...
    * path is the path on the queue
    Note that the priority queue always return the smallest element."""
...
# example queries:
# searcher1 = Searcher(searcheExamples.simp_delivery_graph) # DFS
# searcher1.search() # find the first path
# searcher1.search() # find the next path
# searcher2 = AStarSearcher(searchExample.simp_delivery_graph) # A*
# searcher2.search() # find the first path
